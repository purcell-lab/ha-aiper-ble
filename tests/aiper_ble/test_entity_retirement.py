...
from custom_components.aiper_ble import async_migrate_entry
from custom_components.aiper_ble.config_flow import ConfigFlow
from custom_components.aiper_ble.const import DOMAIN
from custom_components.aiper_ble.datapoints import (
    DEFAULT_ENABLED,
    RETIRED_ENTITY_KEYS,
    SENSOR_NAMES,
)
...
async def test_fresh_install_has_sixteen_entities_and_no_retired_records(hass):
    entry = MockConfigEntry(
    ...
    assert len(items) == 16
    assert sum(item.disabled_by is None for item in items) == 12
